...
const BusinessDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      // Cá nhân hóa owner information
      const userOwner = { 
        id: parseInt(user.id.toString()) || 1, 
        name: user.name || 'Công ty của bạn', 
        email: user.email || '' 
      };
      
      // Mock data cho campaigns với thông tin owner thật
      setCampaigns([
        {
          id: 1,
          title: `${user.name ? user.name + ' - ' : ''}TechStart Innovation 2024`,
          ...
          investors: 67,
          rating: 4.5,
          ownerId: parseInt(user.id.toString()) || 1,
          owner: userOwner,
          status: 'active',
        ...
        {
          id: 2,
          title: `${user.name ? user.name + ' - ' : ''}EcoFarm Organic Solutions`,
        ...
        {
          id: 3,
          title: `${user.name ? user.name + ' - ' : ''}GreenTech Energy`,
      ...
      // Mock data cho recent activity với thông tin cá nhân hóa
      setRecentActivity([
        {
          type: 'investment',
          investor: 'Nguyễn Văn A',
          amount: 50000000,
          campaign: `${user.name ? user.name + ' - ' : ''}TechStart Innovation 2024`,
          time: '2 giờ trước'
        },
        {
          type: 'milestone',
        ...
        },
        {
          ...
          investor: 'Trần Thị B',
          amount: 30000000,
          campaign: `${user.name ? user.name + ' - ' : ''}EcoFarm Solutions`,
          time: '2 ngày trước'
        }
      ]);
    }
  ...
  // Cập nhật business metrics dựa trên campaigns data
  useEffect(() => {
    if (campaigns.length > 0) {
      const totalRaised = campaigns.reduce((sum, campaign) => sum + campaign.raised, 0);
      const activeCampaigns = campaigns.filter(c => c.status === 'active').length;
      const completedCampaigns = campaigns.filter(c => c.status === 'completed').length;
      const totalInvestors = campaigns.reduce((sum, campaign) => sum + (campaign.investors || 0), 0);
      ...
      setBusinessMetrics({
        ...
        totalInvestors,
        avgRaiseTime: 45,
        successRate: Math.round((completedCampaigns / campaigns.length) * 100) || 85
      });
    }
  }, [campaigns]);
            ...
            <div className="flex justify-between items-center py-6">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  Chào mừng trở lại, {user?.name || 'Doanh nghiệp'}!
                </h1>
              ...
              <div className="flex items-center space-x-4">
                ...
                <NotificationSystem userRole="business" userId={user?.id?.toString()} />
              </div>
            </div>
          </div>
        </div>
