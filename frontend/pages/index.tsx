...
const Home: React.FC = () => {
  const [featuredProjects, setFeaturedProjects] = useState<Campaign[]>([]);
          ...
          <div className="text-center mb-8">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
                  <div className="flex items-center mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">AI-powered business intelligence</p>
                    </div>
                  </div>
                  <p className="text-gray-700 mb-4">
                    Test AI campaign optimization, market analysis, và business advisor features.
                  </p>
                  <Link 
                    href="/test-auth"
                ...
                <div className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
                  <div className="flex items-center mb-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">Investor Dashboard</h3>
                      <p className="text-sm text-gray-600">AI-enhanced investment insights</p>
                    </div>
                  </div>
                  <p className="text-gray-700 mb-4">
                    Test AI portfolio optimization, market intelligence, và investment advisor features.
                  </p>
                  <Link 
                    href="/test-auth"
              ...
              <div className="mt-8 p-6 bg-gray-50 rounded-xl">
                <h4 className="text-lg font-semibold text-gray-900 mb-4 text-center">Quick Actions</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Link 
                    href="/test-auth"
                    className="text-center bg-white hover:bg-gray-50 border border-gray-200 rounded-lg p-4 transition-colors"
                  >
                    <div className="text-2xl mb-2">🧪</div>
                    <div className="text-sm font-medium text-gray-900">Test Auth Hub</div>
                    <div className="text-xs text-gray-600">Login & Switch Users</div>
                  </Link>
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-600">Overview & Demo</div>
                  </Link>
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-600">Browse Projects</div>
                  </Link>
                </div>
              </div>
              ...
              <div className="mt-6 text-center">
                <p className="text-sm text-gray-500">
                  💡 <strong>Tip:</strong> Sử dụng "Test Auth Hub" để dễ dàng switch giữa Business và Investor user
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuredProjects.map((project) => {
              ...
              // Dynamic color scheme based on progress percentage
              let progressColorScheme = {
                primary: progress >= 100 ? 'from-green-500 to-green-600' :
                        progress >= 75 ? 'from-blue-500 to-blue-600' :
                        progress >= 50 ? 'from-yellow-500 to-yellow-600' :
                        progress >= 25 ? 'from-orange-500 to-orange-600' : 'from-red-500 to-red-600',
                bg: progress >= 100 ? 'bg-green-100' :
                    progress >= 75 ? 'bg-blue-100' :
                    progress >= 50 ? 'bg-yellow-100' :
                    progress >= 25 ? 'bg-orange-100' : 'bg-red-100',
                text: progress >= 100 ? 'text-green-600' :
                      progress >= 75 ? 'text-blue-600' :
                      progress >= 50 ? 'text-yellow-600' :
                      progress >= 25 ? 'text-orange-600' : 'text-red-600',
                hover: progress >= 100 ? 'hover:shadow-green-200' :
                       progress >= 75 ? 'hover:shadow-blue-200' :
                       progress >= 50 ? 'hover:shadow-yellow-200' :
                       progress >= 25 ? 'hover:shadow-orange-200' : 'hover:shadow-red-200'
              };
              
              // Industry-based color scheme for badges (separate from progress)
              let industryColorScheme = {
              ...
              return (
                <div key={project.id} className="bg-white rounded-3xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 group transform hover:-translate-y-1">
                  <div className="h-56 relative overflow-hidden">
                    {/* Container cho hình ảnh với chiều cao cố định */}
                    <div className="w-full h-full relative">
                      <ImageWithFallback
                        src={project.imageUrl || project.coverImage || '/images/business-meeting.jpg'}
                        alt={`${project.title} - ${project.industry || 'Dự án'}`}
                    ...
                    <div className="absolute inset-0 top-0 left-0 right-0 bottom-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent pointer-events-none"></div>
                    
                    {/* Industry badge */}
                    <div className={`absolute top-4 right-4 ${industryColorScheme.bg} text-blue-900 text-sm font-bold px-3 py-1 rounded-full shadow-lg`}>
                      {project.industry}
                    </div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          ...
                          <div className="flex items-baseline gap-2">
                            <span className={`${progressColorScheme.text} font-bold text-2xl`}>{progress}%</span>
                      ...
                      <div className={`relative w-full h-3 rounded-full overflow-hidden ${progressColorScheme.bg}`}>
                        <div className={`absolute top-0 left-0 h-full bg-gradient-to-r ${progressColorScheme.primary} rounded-full transition-all duration-500`} style={{ width: `${progress}%` }}></div>
                    ...
                    <Link href={`/campaigns/${project.id}`}>
                      <Button variant="primary" className={`w-full py-3 rounded-xl text-white shadow-md ${progressColorScheme.hover} transition-all duration-300`}>
